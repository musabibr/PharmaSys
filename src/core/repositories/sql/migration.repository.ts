...
export class MigrationRepository {
  ...
  async initialise(seedDemo = false, seedHistory = true): Promise<void> {
    ...
    if (seedDemo) {
      const didSeed = await this._seedDemoData();
      if (didSeed && seedHistory) await this._seedHistoricalData();
    }
  ...
  private async _createIndexes(): Promise<void> {
    const indexes = [
      ...
      'CREATE INDEX IF NOT EXISTS idx_purchase_pending_items_purchase ON purchase_pending_items(purchase_id)',
  ...
  private async _runMigrations(): Promise<void> {
    ...
    // Fix YYYY-MM expiry dates (missing day) → YYYY-MM-01
    await this._migrateExpiryDates();
  ...
  /** Fix non-ISO expiry dates that break SQLite date() comparisons (idempotent). */
  private async _migrateExpiryDates(): Promise<void> {
    try {
      // Fix DD/MM/YYYY → YYYY-MM-DD
      const fixed1 = await this.base.runAndGetChanges(
        `UPDATE batches
         SET expiry_date = substr(expiry_date, 7, 4) || '-' || substr(expiry_date, 4, 2) || '-' || substr(expiry_date, 1, 2)
         WHERE expiry_date GLOB '??/??/????'`
      );
      const fixed2 = await this.base.runAndGetChanges(
        `UPDATE purchase_items
         SET expiry_date = substr(expiry_date, 7, 4) || '-' || substr(expiry_date, 4, 2) || '-' || substr(expiry_date, 1, 2)
         WHERE expiry_date IS NOT NULL AND expiry_date GLOB '??/??/????'`
      );
      // Fix YYYY-MM → YYYY-MM-01
      const fixed3 = await this.base.runAndGetChanges(
        `UPDATE batches SET expiry_date = expiry_date || '-01'
         WHERE length(expiry_date) = 7 AND expiry_date GLOB '????-??'`
      );
      const fixed4 = await this.base.runAndGetChanges(
  ...
  private async _seedDemoData(): Promise<boolean> {
  ...
  private async _cleanupSupplierPaymentExpenses(): Promise<void> {
    try {
      // Delete expenses that are linked to purchase_payments (supplier payments should not be expenses)
      const linked = await this.base.getAll<{ id: number }>(
        'SELECT e.id FROM expenses e JOIN purchase_payments pp ON pp.expense_id = e.id'
      );

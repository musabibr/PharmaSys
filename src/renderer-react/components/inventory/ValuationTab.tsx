...
export function ValuationTab() {
  ...
  useEffect(() => {
    fetchData();
    api.categories.getAll().then(d => setCategories(Array.isArray(d) ? d : [])).catch(() => {});
  ...
  const allItems = useMemo(() => {
    ...
    if (categoryId !== '__all__') {
      ...
      filtered = filtered.filter((item) => item.category_id === cid);
    }
    return [...filtered].sort((a, b) => compareItems(a, b, sortKey, sortDir));
  }, [result, search, categoryId, sortKey, sortDir]);

  // Reset page when filters change
  useEffect(() => { setPage(1); }, [search, categoryId, sortKey, sortDir]);
  ...
  const totalProducts = result.total ?? (Array.isArray(result.data) ? result.data.length : 0);

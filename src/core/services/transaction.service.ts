...
export class TransactionService {
  ...
  async createSale(data: CreateTransactionInput, userId: number): Promise<Transaction> {
    ...
    return await this.base.inTransaction(async () => {
      ...
      return await this._commitTransaction(data, lines, userId, shift.id, null);
    });
  }
  ...
  async createReturn(data: CreateReturnInput, userId: number): Promise<Transaction> {
    ...
    // ── 2. Require open shift (returns affect the drawer) ────────────────────
    const shift = await this.shiftRepo.findOpenByUser(userId);
    if (!shift) {
      throw new ValidationError(
    ...
    // ── 2b. 2-shift window — transaction must be from user's last 2 shifts ──
  ...
  private async _deductFIFO(
    ...
    for (const item of items) {
      ...
      if (batches.length === 0) {
        throw new ValidationError(
          `No available stock for product "${product.name}"`, 'stock'
        );
      }
